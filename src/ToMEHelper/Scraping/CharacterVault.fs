    ...
    let rowToCharacter (x: HtmlAgilityPack.HtmlNode) =
        ...
        |> function // columns aren't always consistent
            ...
            | bad ->
                ...
                |> Logging.dumpt "character row failure"

                failwithf
                    "unexpected number of elements %i in %s"
                    bad.Length
                    (bad |> List.map getOuterHtml |> String.concat "")
        |> Option.map (fun (userTd, charTd) ->
            let getHrefWithInner x =
                ...
                |> Option.map (
                    getHrefWithInner
                    >> Tuple2.mapFst (Option.defaultValue null)
                )
    ...
    let clFromRaw
        ...
        | Name n & Level l ->
            ...
            let raceOpt, classOpt =
                ...
                |> Option.map (fun (r, rem) -> r, ToMEHelper.Scraping.ScrapeHelpers.tryGetClass rem)
                |> function
